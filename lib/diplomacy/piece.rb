...
module Diplomacy
  ...
  class Turn::Piece
    ...
    def initialize(turn, type, owner, area, identifier=ID.new)
      ...
      @convoys = []
      @order = nil
      fail unless owner.is_a?(Turn::Power)
      fail unless area.is_a?(Area)
    ...
    def bounced?; @order.bounced? end
    def cut?; @order.cut? end
    def dislodged?; @order.dislodged? end
    ...
    def moving?
      @order.kind_of?(MoveOrder) and @order.successful?
    end

    def approaching?(piece)
      moving? and @order.destination.province == piece.area.province
    end
